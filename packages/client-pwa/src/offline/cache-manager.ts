...
export class OfflineCacheManager {
  ...
  async getContentPack(packId: string): Promise<ContentPackManifest | null> {
    ...
    const result = await this.promisifyRequest(
      store.get(packId) as IDBRequest<ContentPackManifest | undefined>
    );
    return result ?? null;
  }
  ...
  async getLearnerState(learnerId: string): Promise<LearnerState | null> {
    ...
    const result = await this.promisifyRequest(
      store.get(learnerId) as IDBRequest<LearnerState | undefined>
    );
    return result ?? null;
  }
  ...
  async getBufferedEvents(learnerId: string): Promise<LearnerEvent[]> {
    ...
    return this.promisifyRequest(index.getAll(learnerId) as IDBRequest<LearnerEvent[]>);
  }
  ...
  async getAllBufferedEvents(): Promise<LearnerEvent[]> {
    ...
    return this.promisifyRequest(store.getAll() as IDBRequest<LearnerEvent[]>);
  }
  ...
  private async getCacheMeta(key: string): Promise<CacheMeta | null> {
    ...
    const result = await this.promisifyRequest(store.get(key) as IDBRequest<CacheMeta | undefined>);
    return result ?? null;
  }
  ...
  async cleanExpiredCache(): Promise<number> {
    ...
    const allMeta = await this.promisifyRequest(store.getAll() as IDBRequest<CacheMeta[]>);
    const now = new Date();
