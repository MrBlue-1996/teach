...
export class CanonicalFormatter {
  ...
  private formatCode(code: string): string {
    ...
    const normalizedLines = lines.map((line) => {
      ...
      const indentLevel = this.getIndentLevel(leadingWhitespace);
  ...
  /**
   * Infer indentation depth using the configured indent settings
   */
  private getIndentLevel(leadingWhitespace: string): number {
    if (this.config.indentStyle === 'tabs') {
      const tabCount = (leadingWhitespace.match(/\t/g) ?? []).length;
      return tabCount > 0
        ? tabCount
        : Math.floor(leadingWhitespace.length / this.config.indentSize);
    }

    const expandedWhitespace = leadingWhitespace.replace(/\t/g, ' '.repeat(this.config.indentSize));
    return Math.floor(expandedWhitespace.length / this.config.indentSize);
  }
