...
- **[2026-05-26]** `api` `pedagogy` `internal-demo` Session start now seeds teaching mode through `PedagogyEngine.createContext` instead of persisting hardcoded L2. New/updated route tests cover new/low-mastery learner L4 tutorial start, existing non-novice L2 preservation, `recommendedMode` in teach responses, and fade after clean recent trials. Files: `packages/api-server/src/routes/learner.ts`, `packages/api-server/src/routes/learner.test.ts`. Verification: `pnpm --filter @topshelf/api-server test -- src/routes/learner.test.ts`, `pnpm --filter @topshelf/api-server typecheck`.
...
- **[2026-05-26]** `engine` `pedagogy` `internal-demo` Confirmed existing `PedagogyEngine.createContext` precedence is the correct API for session-start seeding: explicit override, initial exposure to L4, block mode, then default. No trigger thresholds changed. Verification: `pnpm --filter @topshelf/engine test` passed.
...
- **[2026-05-26]** `test` `internal-demo` Added focused coverage for the demo-readiness changes: `ReviewNowCard` healthy/watch/urgent/null-date rendering, retention-queue API client contract, session-start seed/fade API behavior, missing-stimulus warning vs release error, and neutral image-stimulus manifest path. Verification includes web, API, engine, shared, and content-authoring targeted suites.
...
- **[2026-05-26]** `frontend` `retention` `pwa` `internal-demo` Extracted dashboard retention affordance into `ReviewNowCard` with minimal props (`affordance`, `href`, optional `className`), preserving healthy omission, watch/urgent styling, and `Open review` copy. Added client test for `learnerApi.getRetentionQueue()`. PWA/mobile fixes include icon precache, fixed theme color, accessible `/kitchen` main target, normal viewport scaling, improved contrast/touch target issues, and quick-action link markup cleanup. Files: `apps/web/src/components/dashboard/ReviewNowCard.tsx`, `apps/web/src/components/dashboard/ReviewNowCard.test.tsx`, `apps/web/src/app/(app)/dashboard/page.tsx`, `apps/web/src/lib/api/learner.test.ts`, `apps/web/public/sw.js`, `apps/web/src/app/layout.tsx`, `apps/web/src/app/kitchen/**`. Verification: web tests/typecheck/build, phone e2e, Lighthouse/viewport/service-worker smoke recorded in `docs/validation/pwa-2026-05-26.md`.

- **[2026-05-26]** `frontend` `api` `pedagogy` `P3.1-followup` Multi-agent review (frontend/engine/api/db/test/quality/infra/content/coordinator) drove a HIGH+MED fix pass. **Server (`packages/api-server/src/routes/learner.ts`)**: seed `TriggerType.HELP_REQUESTED` into `teachingContext.triggers` when a `hint_used` event carries `responseData.helpRequested|helpUsed=true` so L1_MINIMAL learners get `shouldTeach=true`; merge with `detectTriggers` output before `suggestModeElevation`. Retention scheduler also treats prior `hint_used` events in the session as help-aided when client doesn't set `helpUsed` on the `completed` event (defense against client trust gap flagged by quality-reviewer). Tightened `ProgressEventSchema` with `ProgressResponseDataSchema` (helpRequested/helpUsed booleans, `source: 'help_button'|'stuck_timer'`). Two new tests pin the seeding (positive + negative). **Client (`apps/web/src/app/(app)/learn/[courseId]/page.tsx`)**: `STUCK_PROMPT_SECONDS` replaced with `stuckPromptSeconds(estimatedTimeMinutes)` scaling to 40% of block budget (min 180s) — fixes the 15%-of-thinking-time interruption flagged by content-engineer on UJ blocks. Stuck timer gated on `blockAttempts > 0` (requires actual struggle, matches engine STUCK_DETECTED contract). `handleShowHint` + `requestGuidance` wrapped in `useCallback` (stable effect deps). `applyFallback` helper extracted from try/catch duplication. `blockHelpUsedRef` tracks help-used per block and propagates `helpUsed:true` onto the `completed` event for Leitner scheduler. Guidance container gains `role="status" aria-live="polite"`. Attempt badge starts at 1 (not 2), timer moved to `aria-hidden` outside live region. Breakdown panel switched from destructive→warning tokens (breakdown is supportive, not error). All action buttons `size="lg"` (44px touch targets per CLAUDE.md). `handleRetry` now clears guidance. Tests: web 226/226, api-server 318/318 (+2 new), typecheck+lint clean. Tracker P3.1.1-3 → done. Downstream: engine-engineer should evaluate exposing `TriggerThresholds` via block payload so client stops hardcoding (deferred — large refactor).
...
- **[2026-05-26]** `content` `stimulus` `internal-demo` Cleaned stimulus/content contracts for the 8-kind system. `stimulus-synthesis-from-prompts` now documents recursive `--backfill <dir>` behavior and no glob support; content authoring docs clarify missing-stimulus findings warn for demo/internal packs and error for release-mode packs. Validator implements that policy and tests keep image release checks strict: missing manifest image fails, release demo image fails, release authorized image passes. Files: `.agents/skills/stimulus-synthesis-from-prompts/SKILL.md`, `.agents/skills/topshelf-content-pack-authoring/SKILL.md`, `packages/content-authoring/src/validation/content-validator.ts`, `packages/content-authoring/src/index.test.ts`.
...
- **[2026-05-26]** `infra` `pwa` `internal-demo` Ran local production PWA/mobile validation for `/kitchen`. Evidence recorded in `docs/validation/pwa-2026-05-26.md`: `pnpm --filter @topshelf/web build` passed, production server served `/kitchen`, Lighthouse 12 mobile scored 99 performance / 100 accessibility / 100 best practices, service-worker cache smoke confirmed controlled client plus `/kitchen` and `/offline` cached with no private/API paths cached, viewport smoke passed at 320, 375, 768, and 1366x768, and Pixel 7 phone e2e passed 2/2. Remaining blocker: real-device install prompt requires deployed HTTPS URL or tunnel.
...
- **[2026-05-26]** `quality` `internal-demo` Read-only final audit queued after implementation gates. Current known residual risks: real-device PWA install prompt not verified without HTTPS/tunnel; dashboard review CTA intentionally targets the active pack until task-to-pack routing exists; retention remains JSON-backed for internal demo. No release-image guard regression observed in targeted tests.
...
- **[2026-05-26]** `coordinator` `internal-demo` Implemented the internal demo readiness plan across dashboard retention UX, PWA/mobile evidence, worked-example session seeding, stimulus/content contract cleanup, and agent state reconciliation. Evidence is recorded in `.agents/progress.md`, `.github/state/queue.md`, `.github/state/decisions.md`, `.github/state/blockers.md`, and `docs/validation/pwa-2026-05-26.md`. Final gates passed: targeted package checks, `pnpm run workflow:lint`, `git diff --check`, and `pnpm validate`.
