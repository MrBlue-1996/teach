# 4. "Solve First, Teach Second" Pedagogy

## Philosophy

The core teaching philosophy is to maximize learning through struggle and discovery before providing guidance.
...
- Self-discovery is more effective than direct instruction
...
Student encounters problem
    ↓
Attempts solution independently
    ↓
System monitors quietly (L0-L2 mode)
    ↓
Triggers detect if help is needed
```
...
Trigger detected (error, stuck, help requested)
    ↓
Teaching mode determines intervention level
    ↓
Constraint engine filters for device
    ↓
Deliver appropriate guidance
```
...
1. **Trust the Process**: Struggle is learning
2. **Watch for Triggers**: Let the system detect need
3. **Start Conservative**: Begin at L1-L2, elevate if needed
4. **Celebrate Independence**: Acknowledge solving without help
5. **Teach Debugging**: Focus on process, not just answers
